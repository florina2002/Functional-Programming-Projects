...
printMatches :: (TestableMonadIO io) => [FileMatch] -> io ()
printMatches matches = mapM_ (putStrLn . (++ "\n\n") . show) matches
...
readContents Args.Stdin = do
  content <- getContents
  return $ Success [("stdin", content)]
readContents (Args.Files paths) = sequence <$> traverse readFilePath paths
  where
    readFilePath path = fmap (\content -> (path, content)) <$> maybeReadFile path
...
parseContents :: [(FilePath, String)] -> Result MainError [(FilePath, H.Document)]
parseContents fileContents = traverse parseContent fileContents
  where
    parseContent (path, content) =
      case H.parse content of
...
searchFiles query fileDocs = concatMap searchAndMatch fileDocs
  where
    searchAndMatch (path, doc) =
      let matches = search query doc
      in map (FileMatch path) matches
